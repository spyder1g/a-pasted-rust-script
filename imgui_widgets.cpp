...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v) {
        draw->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + (check_bb.Max.y - check_bb.Min.y) / 2), 2, GetColorU32(ImGuiCol_CheckMark));
    }
